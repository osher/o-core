      ...
      , unshift: 
        function ixar_unshift() {
            for (var id, i = 0; i < arguments.length; i++) 
                if ('undefined' != typeof(id = this.idOf(arguments[i])) )
                    this.byId[ id ] = arguments[i];
            return this.base('unshift',arguments);
        }
      , shift: 
        function ixar_shift() {
            var e = this.base('shift',arguments)
              , id
              ;
            if ('undefined' != typeof(id = this.idOf(e)) ) 
                delete this.byId[id];
...
function o_contains(a,b) {
    var isBArr = Array.isArray(b);

    return a === b
        || Object.is(a, NaN) && Object.is(b, NaN)
        || (  'object' == typeof a 
           && 'object' == typeof b
           && ( isBArr &&  Array.isArray(a)
                 ?  //each element in b must be contained in any of elements in a
                    !b.some(function(be) {
                        return be === null 
                            ? -1 == a.indexOf(null)
                            : !a.some(function(ae) { 
                                  return o_contains(ae,be)
                              })
                    })
                 :  //all attributes in b must be contained in a
                    !isBArr && !Object.keys(b).some(function(k) {
                        return !(k in a && o_contains(a[k], b[k]))
                    })
              )
           )
}
